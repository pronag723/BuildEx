...
const ORDER_COLUMNS =
  "id, buyer_id, builder_id, studio_id, assigned_builder_id, building_size, size_label, style, brief, " +
  "price_kopecks, commission_kopecks, builder_earnings_kopecks, " +
  "studio_earnings_kopecks, employee_owed_kopecks, platform_commission_bps_snapshot, employee_commission_bps_snapshot, " +
...
const ORDER_WITH_PARTIES_COLUMNS =
  ...
  ", studio:studio_id (id, name, slug, logo_url)" +
  ", assigned_builder:assigned_builder_id (id, username, display_name, avatar_url)" +
  ", assignments:studio_order_assignments(id, builder_id, employee_commission_bps, assigned_at, released_at, release_reason)";
...
export async function placeStudioOrder({ studioId, size, style, brief }) {
  const supabase = getSupabaseClient();
  if (!supabase) {
    return { orderId: null, error: new Error("Supabase not configured") };
  }
  const { data, error } = await supabase.rpc("place_studio_order", {
    p_studio: studioId,
    p_size: size,
    p_style: style,
    p_brief: brief,
  });
  return { orderId: data || null, error: error || null };
}
...
export async function fetchOrder(orderId) {
  ...
  return { order: rewriteUrlsDeep(data) || null, error: null };
}
