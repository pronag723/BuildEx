...
export async function saveBuilderPayout(supabase, userId, { method, details }) {
  const allowed = new Set(["usdt_trc20", "usdt_erc20", "fiat_card"]);
  const clean = allowed.has(method) ? method : null;
  const trimmed = typeof details === "string" ? details.trim().slice(0, 400) : "";
  if (clean === "fiat_card" && /(?:\d[ -]?){12,19}/.test(trimmed)) {
    return {
      error: {
        message: "Do not enter a full card number. Use a payout reference, email, or last 4 digits only.",
      },
    };
  }
  return upsertBuilderProfile(supabase, userId, {
