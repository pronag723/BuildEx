...
import { BUCKETS, HANDLE_REGEX, sanitizeBuilderTools } from "./constants";
...
export async function saveBuilderExpertise(supabase, userId, payload) {
  ...
  // Both are written only when the caller actually collected them. Builder
  // signup no longer asks for tools or "what kind of work are you open to?" —
  // they are filled in later from the account page — and an unconditional
  // `?? []` here would blank an existing builder's answers every time they
  // saved an unrelated part of this form.
  if (payload.tools !== undefined) {
    patch.tools = sanitizeBuilderTools(payload.tools);
  }
  if (payload.projectTypes !== undefined) {
    patch.project_types = Array.isArray(payload.projectTypes) ? payload.projectTypes : [];
  }
...
export async function saveBuilderStyles(supabase, userId, payload) {
  ...
  // Same guard as saveBuilderExpertise: signup no longer asks for build types,
  // so omitting them must leave an existing builder's list alone rather than
  // clearing it.
...
export async function saveBuilderAvailability(supabase, userId, availabilityStatus) {
  const { error } = await supabase.rpc("set_my_builder_availability", {
    p_status: availabilityStatus ?? "available",
  });
  return { error };
}
