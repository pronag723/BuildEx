...
export async function saveBuilderIdentity(supabase, userId, payload) {
  ...
  // The contact link is re-validated here rather than trusted from the form:
  // buildContactLinks returns {} for anything that isn't a plain handle or an
  // https:// URL, and the CHECK constraint in migration 0098 rejects it a
  // second time server-side.
  const builderPatch = {};
  const editsContactLink =
    payload.contactLinkType !== undefined || payload.contactLinkValue !== undefined;
  if (editsContactLink) {
    const links = buildContactLinks(payload.contactLinkType, payload.contactLinkValue);
    if (payload.contactLinkValue && Object.keys(links).length === 0) {
      return { error: { message: "That contact link isn't in a format we can publish." } };
    }
