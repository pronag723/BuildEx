...
// ─── Account deletion ─────────────────────────────────────────────────────────
// Permanently deletes the signed-in user's auth account. The current
// `delete_own_account` SECURITY DEFINER function (migration 0054) purges Storage,
// detaches managed-studio references, and removes the auth user transactionally.
// The caller is responsible for clearing the local session afterwards.
export async function deleteOwnAccount(supabase) {
  if (!supabase) return { error: { message: "Not connected to Supabase." } };
  const { error } = await supabase.rpc("delete_own_account");
  return {
    error: error ? normalizeError(error, "delete_own_account RPC") : null,
  };
}
