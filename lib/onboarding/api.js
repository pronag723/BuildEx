...
export async function saveBuilderExpertise(supabase, userId, payload) {
  const patch = {
    tools: sanitizeBuilderTools(payload.tools),
  ...
  // Only written when the caller actually collected it. Builder onboarding no
  // longer asks "what kind of work are you open to?", and an unconditional
  // `?? []` here would blank an existing builder's answer every time they saved
  // an unrelated part of this form.
