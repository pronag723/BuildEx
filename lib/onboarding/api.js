...
export async function saveBuilderIdentity(supabase, userId, payload) {
  ...
  const { error: builderErr } = await upsertBuilderProfile(supabase, userId, {
 ...
 */
export async function uploadImage(supabase, { bucket, userId, file, onProgress }) {
  if (!supabase || !userId || !file) {
    return { url: null, error: { message: "Missing arguments for upload" } };
  }

  const path = `${userId}/${randomSegment()}-${Date.now()}.${safeExtension(file)}`;
...
export function uploadAvatar(supabase, userId, file, onProgress) {
  return uploadImage(supabase, { bucket: BUCKETS.avatars, userId, file, onProgress });
}

export function uploadBanner(supabase, userId, file, onProgress) {
  return uploadImage(supabase, { bucket: BUCKETS.banners, userId, file, onProgress });
}

export function uploadPortfolioImage(supabase, userId, file, onProgress) {
  return uploadImage(supabase, { bucket: BUCKETS.portfolios, userId, file, onProgress });
}
