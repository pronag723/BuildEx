...
export async function saveRole(supabase, userId, role) {
  if (!["client", "builder", "both", "studio"].includes(role)) {
...
export async function chooseIndependentBuilderPath(supabase, userId) {
...
// Builder payout preference (Stage 12 payments): how the builder wants to be paid
// once an order completes — `crypto` (USDT wallet) or `card` (off-ramp contact).
// `details` carries the wallet address or contact string. These columns are
// granted to authenticated in migration 0031; they hold no economic privilege.
export async function saveBuilderPayout(supabase, userId, { method, details }) {
  const allowed = new Set(["usdt_trc20", "usdt_erc20"]);
  const clean = allowed.has(method) ? method : null;
  const trimmed = typeof details === "string" ? details.trim().slice(0, 400) : "";
  return upsertBuilderProfile(supabase, userId, {
    payout_method: clean,
    payout_details: clean ? trimmed || null : null,
  });
}
...
export async function deleteOwnAccount(supabase) {
  ...
  if (error) {
    ...
    try {
      // FunctionsHttpError.message is always the unhelpful "non-2xx status
      // code". The Edge Function returns the actual database/storage failure
      // as JSON on the Response stored in `context`.
      const response = error?.context?.clone?.() || error?.context;
      const body = await response?.json?.();
      serverMessage = typeof body?.error === "string" ? body.error : null;
    } catch {
      // A proxy or network failure may not have a JSON response body.
    }
    const unavailable =
      status === 404 ||
      error?.name === "FunctionsFetchError" ||
      /failed to send a request|fetch failed/i.test(error?.message || "");

    // Older deployments may not have the Edge Function yet. The database RPC
    // still performs studio-aware deletion, so an unavailable function must
    // not trap the account.
    if (unavailable) {
      const { error: rpcError } = await supabase.rpc("delete_own_account");
      return {
        error: rpcError ? normalizeError(rpcError, "delete_own_account RPC") : null,
      };
    }
  ...
  if (!data?.deleted) return { error: { message: data?.error || "Could not delete account." } };
  return { error: null };
}
