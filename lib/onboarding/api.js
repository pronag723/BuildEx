...
// ─── Role ────────────────────────────────────────────────────────────────────
// Marks a visitor as opting in to being a builder. `role` is legacy — nothing
// reads it to decide builder-ness any more — but keeping it truthful means
// existing accounts and any leftover role-aware copy stay coherent. A user who
// was a "client" becomes "both"; anything else already covers building.
export async function markRoleAsBuilder(supabase, userId, currentRole) {
  const next =
    currentRole === "client" ? "both" : currentRole ? null : "builder";
...
/**
 * Step 1 — identity. Writes the public profile fields (display name, @handle,
 * avatar, bio) and, on builder_profiles, the optional contact link.
 *
 * Every field is written only when the caller actually collected it: this is
 * also the account-settings writer, where one card saves the bio and another
 * saves the name, and an unconditional `?? null` would blank whatever the other
 * card owns. `banner_url` is never written at all — banners are gone from the
 * product, and the stored value stays as it is.
 */
export async function saveBuilderIdentity(supabase, userId, payload) {
  ...
  if (payload.avatarUrl !== undefined) profileUpdate.avatar_url = payload.avatarUrl ?? null;
  if (payload.bio !== undefined) profileUpdate.bio = payload.bio ?? null;
  ...
  // The links are re-validated here rather than trusted from the form:
  // buildContactLinks drops every row that isn't a recognised handle or a
  // plain https:// URL on that platform's own host, and the CHECK constraint
  // in migration 0098/0099 rejects them a second time server-side.
  const builderPatch = {};
  if (payload.contactLinks !== undefined) {
    const rows = Array.isArray(payload.contactLinks) ? payload.contactLinks : [];
    const links = buildContactLinks(rows);
    const filledRows = rows.filter((r) => String(r?.value || "").trim()).length;
    if (filledRows > 0 && Object.keys(links).length < filledRows) {
      return { error: { message: "One of your links isn't in a format we can publish." } };
    }
    builderPatch.contact_links = links;
  }

  // Touch builder_profiles only when there is builder data to write, or when
  // the caller is signup and needs the row to exist for the later steps.
  // Being a builder IS having this row, so creating one for a visitor who is
  // only renaming themselves on /account would silently list them publicly.
  if (!payload.ensureBuilderRow && Object.keys(builderPatch).length === 0) {
    return { error: null };
  }
...
/**
 * Step 2 — the styles this builder works in. `build_types` is only written when
 * the caller passes it; signup never does, so an older builder's stored list
 * survives untouched.
 */
export async function saveBuilderStyles(supabase, userId, payload) {
  ...
  if (payload.buildTypes !== undefined) {
    patch.build_types = Array.isArray(payload.buildTypes) ? payload.buildTypes : [];
  }
  return upsertBuilderProfile(supabase, userId, patch);
}
...
// ─── Cancel builder onboarding ───────────────────────────────────────────────
// Discards the half-built BUILDER profile, leaving the user's account exactly as
// a one-click sign-in produces it: signed in, with their handle, display name,
// avatar and bio intact. Being a builder is an opt-in now, so backing out of it
// is not the same thing as abandoning an account — the visitor row is a real,
// usable account and must survive. We delete the builder_profiles row and the
// portfolio images (both of which the owner *can* remove under RLS) and leave
// public.profiles alone apart from clearing the legacy `role` marker when it
// only said "builder".
export async function cancelOnboarding(supabase, userId, currentRole = null) {
  ...
  // `role` is legacy and no longer decides anything (builder-ness is the
  // existence of a builder_profiles row), but leaving a stale "builder" behind
  // on someone who just backed out is misleading. "both" degrades to "client";
  // any other value is the user's own and is left untouched.
  const nextRole =
    currentRole === "builder" ? null : currentRole === "both" ? "client" : undefined;
...
// files through the supported Storage API, then migration 0055's
// `delete_own_account` RPC detaches managed-studio references and removes the
// auth user transactionally. The caller clears the local session afterwards.
export async function deleteOwnAccount(supabase) {
  ...
  if (error) {
    const normalized = normalizeError(error, "delete-account Edge Function");
    let serverMessage = null;
    try {
      // FunctionsHttpError.message hides the useful JSON returned by the
      // function. Read it from the Response kept in `context`.
      const response = error?.context?.clone?.() || error?.context;
      const body = await response?.json?.();
      serverMessage = typeof body?.error === "string" ? body.error : null;
    } catch {
      // Network/proxy failures may not include a JSON response.
    }
    return {
      error: {
        ...normalized,
        status: error?.context?.status ?? normalized?.status ?? null,
        message: serverMessage || normalized?.message || "Could not delete account.",
      },
    };
  }
  if (!data?.deleted) {
