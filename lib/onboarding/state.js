...
export function resolveNextStep(profile, builderProfile, portfolioCount = 0) {
  ...
  if (isBuilder(profile.role)) {
    ...
    // A studio sets the customer-facing price. Employees therefore never set
    // independent rates during registration (and their portfolio is optional).
    // Check this before rate completeness so a fresh employee is never routed
    // to the independent pricing screen.
    if (builderProfile.pending_employee_code) return STEPS.builderStudioComplete;
