...
export const STEPS = {
  ...
  builderStudio:      "/onboarding/builder/studio",
  builderExpertise:   "/onboarding/builder/expertise",
  builderStyles:      "/onboarding/builder/styles",
  builderRates:       "/onboarding/builder/rates",
  builderPortfolio:   "/onboarding/builder/portfolio",
  builderStudioComplete: "/onboarding/builder/studio/complete",
  studioSetup:        "/onboarding/studio",
  complete:           "/onboarding/complete",
};

const VALID_ROLES = new Set(["client", "builder", "both", "studio"]);
...
export function resolveNextStep(profile, builderProfile, portfolioCount = 0) {
  ...
  if (profile.role === "studio") return STEPS.studioSetup;
  ...
  if (isBuilder(profile.role)) {
    if (profile.role === "builder" && !builderProfile) {
      return STEPS.builderStudio;
    }
    if (
      !builderProfile ||
    ...
    // A builder who joins with a studio employee code is staffed by the
    // studio, rather than listed as an independent provider. They only need a
    // usable identity (name + handle); prices, public availability, expertise
    // and portfolio belong to the studio workflow and must not block joining.
    if (builderProfile.pending_employee_code) {
      return STEPS.builderStudioComplete;
    }
    ...
    const rates = builderProfile.rates;
    const hasRates =
      rates && typeof rates === "object" && Object.keys(rates).length > 0;
    if (!hasRates) return STEPS.builderRates;
...
export function stepProgress(stepPath, role) {
  ...
  const studioOrder = [STEPS.role, STEPS.studioSetup, STEPS.complete];
  if (role === "studio") {
    const i = studioOrder.indexOf(stepPath);
    return i < 0 ? 0 : i / (studioOrder.length - 1);
  }
...
export function stepsForRole(role) {
  if (role === "studio") {
    return [
      ...
      { path: STEPS.studioSetup, label: "Studio" },
    ];
  }
  if (isBuilder(role)) {
    return [
