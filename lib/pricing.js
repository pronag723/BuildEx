...
// BuildEx now pins buyer checkout to USDT on TRON (`usdttrc20`) to keep network
// costs and gateway minimums low for this niche. NOWPayments still documents the
// exact minimum as dynamic, so the Edge Function performs the provider-side
// check at invoice creation time. The UI/DB floor stays at $20 as a marketplace
// rule; if NOWPayments raises the live minimum above that, checkout rejects the
// order until the builder raises the price.
export const MIN_ORDER_CENTS = 2000; // $20.00

// Default display order for the built-in tiers when a row predates `pos`.
const BUILTIN_POS = { small: 0, medium: 1, large: 2 };

// Icon shown for builder-added custom tiers (lib/icons.jsx key).
export const CUSTOM_TIER_ICON = "package";
...
export const PREVIEW_RADIUS_MAX = 512;
 ...
 * Returns: [{ id, label, icon, hint, areaText, blocks, price (cents), enabled }] sorted
  ...
  const tiers = Object.entries(rates).map(([id, raw], index) => {
    ...
    return {
      ...
      areaText:
        Number(v.blocks) > 0
          ? meta?.areaLabel
            ? meta.areaLabel(Number(v.blocks))
            : `Up to ${Number(v.blocks)}×${Number(v.blocks)} blocks`
          : "Custom area",
      price,
      enabled,
      pos,
