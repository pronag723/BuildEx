...
export async function ensureProfile(supabase, user) {
  ...
  try {
    ...
    if (existing) {
      ...
      return { profile: existing, created: false, skipped: false };
    }
    ...
    if (insertError) {
      ...
      if (refetched) {
        return { profile: refetched, created: false, skipped: false };
      }
    ...
    if (inserted) {
      ...
      return { profile: inserted, created: true, skipped: false };
    }
    ...
    const { data: refetched } = await supabase
      ...
      .eq("id", user.id)
      .maybeSingle();
    return { profile: refetched || null, created: false, skipped: false };
  } catch (error) {
    // eslint-disable-next-line no-console
...
export function displayInfoFromUser(user, profile) {
  if (!user) return null;
  const meta = pickMetadata(user);
  // The profile row is the source of truth: a builder/client who has chosen a
  // custom avatar should always see it. Only fall back to the OAuth provider's
  // avatar (e.g. Discord) when they haven't set one. `trim()` guards against an
  // empty string sneaking in and silently falling through to the provider.
  ...
  return {
    ...
    avatarUrl: chosenAvatar || meta.avatarUrl,
