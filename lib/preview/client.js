...
// onProgress(value 0..1) is called as work proceeds. `opts` is forwarded to
// buildPreview ({ center: { x, z, y? }, radius }) so the builder can scope an
// infinite/terrain world to the build's coordinates. Returns { bytes, meta }.
export async function generatePreview(file, onProgress, opts = {}) {
  ...
  const { bytes, meta } = buildPreview(zipBytes, onProgress, opts);
  return { bytes, meta };
}
