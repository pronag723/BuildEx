...
async function invokePayoutFunction(name, body) {
  const supabase = getSupabaseClient();
...
export function createPayoutBatch(payoutIds) {
  return invokePayoutFunction("create-payout", { payoutIds });
}

export function verifyPayoutBatch(batchId, code) {
  return invokePayoutFunction("verify-payout", { batchId, code });
}

export function reconcilePayoutBatch(batchId) {
  return invokePayoutFunction("reconcile-payout", { batchId });
}
