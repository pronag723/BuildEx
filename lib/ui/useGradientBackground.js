...
// Shared animated-gradient background used across the catalog/account
...
// `refreshKey` lets pages that initially render a loading shell restart the
// effect once their gradient nodes mount with the loaded content.
export function useGradientBackground(refreshKey) {
  ...
  useEffect(() => {
    ...
    if (!bg || !edge || window.getComputedStyle(bg).display === "none") return undefined;
