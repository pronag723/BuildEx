...
export async function listMyReadyBuilds() {
  const supabase = getSupabaseClient();
  if (!supabase) return { listings: [], error: null };
  const { data, error } = await supabase.from("ready_builds").select("id,title,description,style,price_kopecks,is_active,created_at,current_version_id,media:ready_build_media(id,url,alt,position),purchases:ready_build_purchases(id,status,builder_earnings_kopecks)").order("created_at", { ascending: false });
  return { listings: rewriteUrlsDeep(data || []), error: error || null };
}
...
export async function uploadReadyBuildImage(listingId, file, position) {
  ...
  const { error } = await supabase.from("ready_build_media").insert({ listing_id: listingId, storage_path: path, url: urlData.publicUrl, alt: file.name, position });
  return { error: error || null };
}
...
export async function createReadyBuildInvoice(purchaseId, payCurrency) {
  ...
  const returnUrl = typeof window !== "undefined" ? `${window.location.origin}${process.env.NEXT_PUBLIC_BASE_PATH || ""}/account/?section=purchases` : undefined;
