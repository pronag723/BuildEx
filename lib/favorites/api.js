...
export async function listFavorites() {
  ...
  const { data, error } = await supabase
    .from("favorites")
    .select("builder_id, studio_id, ready_build_id")
    .order("created_at", { ascending: false });

  if (error) return { builderIds: [], favoriteKeys: [], error };
  const favoriteKeys = (data || []).map((row) => {
    if (row.ready_build_id) return `ready_build:${row.ready_build_id}`;
    if (row.studio_id) return `studio:${row.studio_id}`;
    return `builder:${row.builder_id}`;
  });
  return {
...
export async function addFavorite(userId, targetId, providerType = "builder") {
  ...
  const { error } = await supabase
    ...
    .insert({
      user_id: userId,
      builder_id: providerType === "builder" ? targetId : null,
      studio_id: providerType === "studio" ? targetId : null,
      ready_build_id: providerType === "ready_build" ? targetId : null,
    });
...
export async function removeFavorite(userId, targetId, providerType = "builder") {
  ...
  query =
    providerType === "studio"
      ...
      : providerType === "ready_build"
        ? query.eq("ready_build_id", targetId)
