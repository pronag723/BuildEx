...
// route the SDK through a non-Cloudflare reverse proxy (Caddy on an Oracle Cloud
// Always-Free VM; NEXT_PUBLIC_SUPABASE_URL points at its host). New uploads
// already use the proxy host, but rows uploaded before the swap still embed the
// old absolute URL. Rewriting at render time fixes the legacy data without a DB
// migration, and remains correct if the proxy host ever changes.
...
const URL_FIELD_KEYS = new Set([
  ...
  "banner_url",
  "other_avatar_url",
  // admin_list_conversation_reports / admin_get_conversation_messages flatten
  // the joined profile rows, so their avatars arrive under prefixed keys.
  "reporter_avatar_url",
  "sender_avatar_url",
  "thumbnail",
  "preview_url",
