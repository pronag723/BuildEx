...
//   1 bp = 0.01% → 1800 bps = 18%.
// Schedule raised in migration 0026 (Studios) so margin survives the payment-
// gateway processing cost + the studio override on every tier — keep these in sync with
// commission_bps_for_rank in 0026_studios.sql.
export const RANK_RULES = {
  rookie:   { key: "rookie",   commissionBps: 1800, minCompleted: 0,  minRating: 0 },
  advanced: { key: "advanced", commissionBps: 1500, minCompleted: 5,  minRating: 4.0 },
  expert:   { key: "expert",   commissionBps: 1200, minCompleted: 12, minRating: 4.5 },
  master:   { key: "master",   commissionBps: 900,  minCompleted: 22, minRating: 4.8 },
};
...
export function rankProgress({ rank, completedOrders = 0, avgRating = 0 } = {}) {
  ...
  return {
    ...
    ordersPct: rule.minCompleted > 0
      ...
      : 1,
    // 0–1 fraction of the way to the rating threshold, for a progress bar
    // mirroring the completed-orders one. Clamped to [0,1]; a 0-threshold rank
    // (none today, but future-proof) reads as already met.
    ratingPct: rule.minRating > 0
      ? Math.min(1, ratingHave / rule.minRating)
      : 1,
  };
}
