...
export async function listAdminOrders(filter = "all", query = "") {
  const supabase = getSupabaseClient();
  if (!supabase) return { orders: [], error: null };

  let { data, error } = await supabase.rpc("admin_list_orders", {
    p_filter: filter,
    p_query: query.trim() || null,
  });
  if (error && query.trim() === "") {
    ({ data, error } = await supabase.rpc("admin_list_orders", { p_filter: filter }));
  }
...
export async function getAdminUserOrders(userId) {
  const supabase = getSupabaseClient();
  if (!supabase || !userId) return { orders: [], error: null };

  const { data, error } = await supabase.rpc("admin_get_user_orders", {
    p_user: userId,
  });
  return { orders: error ? [] : data || [], error: error || null };
}
