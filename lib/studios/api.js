...
export const STUDIO_COLUMNS =
  "id, name, slug, logo_url, about, bio, status, moderator_id, rates, " +
  "platform_commission_bps, employee_commission_bps, accepting_orders, " +
  "available_employees, payout_method, payout_details, " +
  "created_at, claimed_at";

const PUBLIC_STUDIO_COLUMNS =
  "id, name, slug, logo_url, about, bio, status, rates, accepting_orders, available_employees, " +
  "created_at, claimed_at";
...
export async function searchIndependentBuilders(query) {
  ...
  const { data, error } = await supabase.rpc("search_independent_builders_v2", {
    p_query: String(query || "").trim(),
  });
  if (!error) return { builders: data || [], error: null };

  // Older deployments may not have the latest search function yet. Fall back
  // to the public profile tables so a removed builder remains discoverable while
  // the migration is being applied.
  let fallback = supabase
    .from("profiles")
    .select("id, username, display_name, avatar_url, bio, builder_profile:builder_profiles!inner(*)")
    .in("role", ["builder", "both"])
    .not("username", "is", null)
    .eq("builder_profile.profile_type", "independent");
  const normalized = String(query || "").trim();
  if (normalized) {
    fallback = fallback.or(`username.ilike.%${normalized}%,display_name.ilike.%${normalized}%`);
  }
  const { data: rows, error: fallbackError } = await fallback
    .order("display_name", { ascending: true, nullsFirst: false })
    .limit(20);
  if (fallbackError) return { builders: [], error };

  const { data: studio } = await supabase.rpc("get_my_managed_studio");
  let pendingByBuilder = new Map();
  if (studio?.id) {
    const { data: pending } = await supabase
      ...
      .eq("status", "pending");
    pendingByBuilder = new Map((pending || []).map((item) => [item.builder_id, item]));
  }
  ...
  return {
    builders: (rows || []).map((row) => ({
      ...
      pending_invitation_id: pendingByBuilder.get(row.id)?.id || null,
      pending_invitation_status: pendingByBuilder.get(row.id)?.status || null,
    })),
    error: null,
  };
}
...
export async function listMyStudioInvitations() {
  ...
  const { data, error } = await supabase.rpc("list_my_studio_builder_invitations");
  if (!error) return { invitations: rewriteUrlsDeep(data || []), error: null };

  // Keep the invitations page usable on projects that have not refreshed the
  // schema cache for migration 0061 yet. The richer fields are populated once
  // the RPC is available; the legacy projection still supports accept/decline.
  const legacy = await supabase
    ...
    .order("created_at", { ascending: false });
  return {
    invitations: legacy.error ? [] : rewriteUrlsDeep(legacy.data || []),
...
export async function createStudioBuilderInvitation(builderId) {
  ...
  let { data, error } = await supabase.rpc("create_studio_builder_invitation_v2", {
    p_builder: builderId,
  });
  if (error) {
    // Backward-compatible fallback for projects before migration 0061. The
    // versioned function is required for re-inviting removed builders, but the
    // legacy function still supports ordinary invitations during rollout.
    const legacy = await supabase.rpc("create_studio_builder_invitation", {
      p_builder: builderId,
    });
    data = legacy.data;
    error = legacy.error;
...
export async function respondToStudioBuilderInvitation(invitationId, response) {
  const supabase = getSupabaseClient();
  if (!supabase) return { error: clientError() };
  const { data, error } = await supabase.rpc("respond_to_studio_builder_invitation", {
    p_invitation: invitationId,
    p_response: response,
  });
  if (!error && data?.cleanup_job_id) {
    // Membership acceptance is already committed. Cleanup is durable and may
    // safely be retried if this best-effort call is interrupted.
    await supabase.functions.invoke("cleanup-ready-build-assets", {
      body: { jobId: data.cleanup_job_id },
    });
  }
  return { result: data || null, error: error || null };
}

export async function getMyStudioLeaveEligibility() {
  const supabase = getSupabaseClient();
  if (!supabase) return { eligibility: null, error: clientError() };
  const { data, error } = await supabase.rpc("get_my_studio_leave_eligibility");
  return { eligibility: data || null, error: error || null };
}

export async function leaveMyStudio() {
  const supabase = getSupabaseClient();
  if (!supabase) return { error: clientError() };
  const { error } = await supabase.rpc("leave_my_studio");
  return { error: error || null };
}

export async function getMyStudioDeleteEligibility() {
  const supabase = getSupabaseClient();
  if (!supabase) return { eligibility: null, error: clientError() };
  const { data, error } = await supabase.rpc("get_my_studio_delete_eligibility");
  return { eligibility: data || null, error: error || null };
}
...
export async function removeStudioEmployee(builderId, confirmation) {
  const supabase = getSupabaseClient();
  if (!supabase) return { error: clientError() };
