...
// of throwing. Rows were inserted server-side by the order/review/dispute RPCs
// (via _notify); those producers are decommissioned, so the table is read-only
// history now — the client only ever reads its own rows and marks them read.
// ─────────────────────────────────────────────────────────────────────────────
...
// Delete every one of the caller's notifications ("Clear all"). RLS + the
// owner-delete policy in migration 0017 guarantee a user can only ever remove
// their own rows. The `.neq("id", …)` is just a guard so Supabase doesn't reject
// an unfiltered DELETE; RLS still scopes it to the caller. Returns { error }.
