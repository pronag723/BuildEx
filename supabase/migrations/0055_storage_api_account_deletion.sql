-- Supabase Storage forbids deleting rows from storage.objects directly. The
-- delete-account Edge Function already removes the caller's files through the
-- supported Storage API, so the database transaction must only detach retained
-- managed-studio references and delete the auth user.
